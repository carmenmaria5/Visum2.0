...
public class Base extends AppCompatActivity implements View.OnTouchListener, CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    static {
        if (!OpenCVLoader.initDebug()) Log.d("MAL", "No se ha podido cargar OpenCV");
        else Log.d("BIEN", "OpenCV se ha cargado");
    }
...
//        tvTouchCoordenadas.setText("X: " + Math.round(Double.valueOf(x)) + ", Y: " + Math.round(Double.valueOf(y)));
        ...
        String hex = "#" + String.format("%02X", (int) tColorRGBa.val[0]) + String.format("%02X", (int) tColorRGBa.val[1]) + String.format("%02X", (int) tColorRGBa.val[2]);
...
//        tvTouchColor.setText("Color Hex: "+hex+"  Color RGB: "+ rgb);
    ...
    public void onPause() {
        super.onPause();
        if (OpenCvCamara != null) OpenCvCamara.disableView();
    }
